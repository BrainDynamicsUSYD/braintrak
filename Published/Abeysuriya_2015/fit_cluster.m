function fdata = fit_cluster(model,dataset,subject_idx,npts_per_fit,suffix)
	...
	if exist(sprintf('%s/cached_output.mat',output_dir)) % Resume an interrupted run
		...
		[~,fit_data,plot_data] = bt.core.fit_spectrum(model,d.f(:),target_P,initial_pp,initial_params,npts_per_fit,target_state{initial_idx},[],debugmode);
	...
	for j = j_start:length(t_increment)
		...
		if d.nspec(didx) < 20 
			...
			try
				[~,fit_data,plot_data] = bt.core.fit_spectrum(model,d.f(:),target_P,fit_data.posterior_pp,fit_data.fitted_params,npts_per_fit,d.state_str{didx});	
				fprintf('Fitted %i\n',j);
			catch
				fit_data.skip_fit(1:end) = 4;
				fit_data.target_f = d.f(:);
				fit_data.target_P = target_P;
				fprintf('Skipping %i due to an error in bt.core.fit_spectrum()\n',j);
			end
		end
