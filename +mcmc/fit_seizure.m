function fdata = fit_seizure(subject_idx,model,npts_per_fit)
	...
	d = mcmc.load_seizure_data(subject_idx,'Cz',30,4); 
	st(6) = 1200;
	st(10) = 280;
	st(11) = 720;
	...
	for j = 1:max_iters
		...
		if j == 1
			...
			[fit_data(j),plot_data(j)] = mcmc.fit(model,d.f(:),target_P,initial_pp,initial_params,npts_per_fit,[],[],debugmode);
			fdata = mcmc.feather(model,fit_data(j),plot_data(j));
		else
			[fit_data(j),plot_data(j)] = mcmc.fit(model,d.f(:),target_P,fit_data(j-1).posterior_pp,fit_data(j-1).fitted_params,npts_per_fit,[],[],debugmode);			
