classdef full_gdiff < mcmc.model.template
	...
	methods
		function self = full_gdiff() % Constructor
			self.name = 'model_full';
			self.n_params = 9;
			self.param_names = {'Gee','Gee+Gei','Gese','Gesre','Gsrs','Alpha','Beta','t0','EMGa'};
			self.param_symbols = {'G_{ee}','G_{ee}+G_{ei}','G_{ese}','G_{esre}','G_{srs}','\alpha','\beta','t_0','A_{EMG}'};
			self.param_units = {'','','','','','ms','ms','ms',''};
			
			self.n_fitted = 9;
			self.skip_fit = zeros(1,self.n_fitted);

			self.initial_step_size = [1  0.4  0.15  0.4  0.2  5  40  0.005  0.005];
		   	self.limits = [ eps  -20        eps     -40      -14      10      100    0.075   0  ;...
		      		        20   2       40      -eps      -eps      100    800    0.14   1  ];
		...
		function p = p_from_params(self,fitted_params) % Map parameters to point
			...
			p.gabcd(2) = fitted_params(2)-fitted_params(1);
		...
		function valid = validate_params(self,pars)
			valid = ~(pars(1)/gei > -0.5 || (pars(1) + gei) > 1 || pars(7)/pars(6) > 20 || any(pars > self.limits(2,:) | pars < self.limits(1,:) ));
		end

		function [chisq,P] = objective(self,pars) % Calculate the objective
			gei = pars(2)-pars(1); % Convert gee+gei into gei
			P = [];
			chisq = NaN;
		    ...
		    d=(stab_gamma_prefactor.*(1-stab_L.*gei)-stab_L.*pars(1)).*(1-stab_L.*stab_L.*pars(5))-stab_L.*stab_L.*self.stab_Mtot.*(pars(3) + stab_L.*pars(4));      
		...
		function [initial_values,prior_pp] = initialize_fit(self,target_f,target_P) % Return the first posterior/prior and initial values
			...
			initial_values =  [a(1) a(1)+a(2) a(3)*a(4) a(3)*a(5)*a(7) a(5)*a(8) p.alpha(1) p.beta(1) p.t0 0];
		...
		function xyz = get_xyz(self,params) % Turn whatever model parameters are specified into XYZ - params is a matrix
			xyz(:,1) = params(:,1)./(1-(params(:,2)-params(:,1)));
