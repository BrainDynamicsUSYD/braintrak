classdef reduced_l < mcmc.model.template
	...
	%
	% This model includes the factors of L in other connections
	...
	methods
		function self = reduced_l() % Constructor
			self.name = 'model_reduced_l';
			self.n_params = 8;
			...
			%self.initial_step_size = 0.05.*[0.2 0.2 0.2 0.2 5 10 0.02 0.05];
			self.initial_step_size = [0.4  0.4 0.1 0.1 20  100  0.002  1]; % GOOD

		   	self.limits = [ eps  -20   -1   eps       10    100    0.075   0  ;...
		      		        20   -eps   1     1      100    800    0.14   5  ];
		...
		function p = p_from_params(self,param_values) % Map parameters to point
			p = model.params;
			p.gabcd(1) = param_values(1);
			p.gabcd(2) = param_values(2);

			p.xyz      = self.get_xyz(param_values);
			p.alpha(:) = param_values(5);
			p.beta(:)  = param_values(6);
			p.t0       = param_values(7);
			p.taues    = p.t0/2;
			p.tause    = p.t0/2;
			p.emg_a    = param_values(8);
		end
		
		function params = params_from_p(self,p) % Map point to parameters
			params = [p.gabcd(1:2) p.xyz(2:3) p.alpha(1) p.beta(1) p.t0 p.emg_a];
		end
		...
		function [chisq,P] = objective(self,pars) % Calculate the objective
		    ...
		    if self.compute_Mtot % If this flag has been set to false, then these fields are already present
		        ...
		        self.stab_Mtot = exp(1i*self.stab_w*pars(7));
		        self.spec_Mtot = exp(1i*self.spec_w*pars(7));
		    end

		    stab_L = 1./((1-1i*self.stab_w/pars(5)).*(1-1i*self.stab_w/pars(6)));
		    spec_L = 1./((1-1i*self.spec_w/pars(5)).*(1-1i*self.spec_w/pars(6)));
		    zprime = (pars(5)+pars(6))^2/(pars(5)*pars(6))*pars(4); % Effectively -Gsrs: note that this is POSITIVE

		    % cf. James corticothalamic dynamics eq 9
		    d = (self.stab_gamma_prefactor.*(1-stab_L.*pars(2))-stab_L.*pars(1)).*(1+stab_L.*stab_L.*zprime) - stab_L.*stab_L.*pars(3).*(1-pars(2)).*(1+zprime).*self.stab_Mtot;
		    ...
		    q2re2 = self.spec_gamma_prefactor - spec_L*pars(1)./jei_oneminus -spec_L.*spec_L.*(pars(3)*(1+zprime)*(1-pars(2)))./one_plus_zprime./jei_oneminus.*self.spec_Mtot;
		    k2u = self.k2u;
		    k2_volconduct = self.k2_volconduct;
		    re2 = self.p.re.^2;
		    T_prefactor = spec_L.*spec_L./(jei_oneminus.*one_plus_zprime);

		    for j = 1:size(k2u,1)
		        P = P + k2u(j,2).*abs(T_prefactor./(k2u(j,1)*re2+q2re2)).^2 * k2_volconduct(j); % For use with the efficient way
		    end

		    P = P + 5e-4*pars(8)*self.emg; % Add the EMG component
		...
		function [initial_values,prior_pp] = initialize_fit(self,target_f,target_P) % Return the first posterior/prior and initial values
			...
			initial_values =  [b(1) b(2) a(2) a(3) p.alpha(1) p.beta(1) p.t0 0.05];
		...
		function set_cache(self,initial_values)
			...
			if self.skip_fit(6) == 2 && ~isempty(initial_values) % If t0 is not being fitted, then the value is of course fixed
				self.stab_Mtot = exp(1i*self.stab_w*initial_values(7));
				self.spec_Mtot = exp(1i*self.spec_w*initial_values(7));
		...
		function prepare_for_fit(self,varargin)
			...
			if nargin < 6 || isempty(varargin{5})
				self.default_t0_skip_fit(varargin{1},varargin{2},7);
			else
