classdef spatial_express_t0_noemg < mcmc.model.template_spatial
	...
	methods
		function self = spatial_express_t0_noemg(n_modes) % Constructor
			self.name = 'spatial_express_t0_noemg';
			self.n_params = 9;
			...
			self.initial_step_size = 0.05.*[2 2 2 2 2 5 10 0.02 0.05];
			self.limits = [
				eps       	20     	  %Gee
				-20     	-eps      %Gei
				eps       	40     	  %Gese
		...
		function set_params(self,pars)
			...
			self.p.emg_a = 0;
			self.p.spatial_t0 = self.p.variation_function('cosine',self.p.t0,pars(9));
		end
