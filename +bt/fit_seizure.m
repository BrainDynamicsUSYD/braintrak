function fdata = fit_seizure(subject_idx,model,npts_per_fit)
	...
	for j = 1:max_iters
		...
		if j == 1
			...
			[fit_data(j),plot_data(j)] = bt.fit(model,d.f(:),target_P,initial_pp,initial_params,npts_per_fit,[],[],debugmode);
			fdata = bt.feather(model,fit_data(j),plot_data(j));
		else
			[fit_data(j),plot_data(j)] = bt.fit(model,d.f(:),target_P,fit_data(j-1).posterior_pp,fit_data(j-1).fitted_params,npts_per_fit,[],[],debugmode);			
