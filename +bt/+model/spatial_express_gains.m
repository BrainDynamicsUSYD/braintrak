classdef spatial_express_gains < bt.model.template_spatial
	...
	methods
		function self = spatial_express_gains(n_modes) % Constructor
			...
			self.n_params = 15;
			self.param_names = {'Gee','Gei','Gese','Gesre','Gsrs','Alpha','Beta','t0','EMGa','t0_amp','t0_phase','Gee_amp','Gee_phase','Gese_amp','Gese_phase'};
			self.param_symbols = {'G_{ee}','G_{ei}','G_{ese}','G_{esre}','G_{srs}','\alpha','\beta','t_0','A_{EMG}','t_{0amp}','t_{0\phi}','Gee_{amp}','Gee_{\phi}','Gese_{amp}','Gese_{\phi}'};
			self.param_units = {'','','','','','s^{-1}','s^{-1}','ms','','ms','m','','','',''};

			self.n_fitted = self.n_params;
			self.skip_fit = zeros(1,self.n_fitted);
			self.initial_step_size = [0.4  0.4  1  1  0.2  5  40  0.005 0.05 0.005 0.2 0.1 0.2 0.1 0.2];
			self.limits = [
				eps       	20     	  %Gee
				-20     	-eps      %Gei
				eps       	40     	  %Gese
		...
		function [initial_values,prior_pp] = initialize_fit(self,target_f,target_P) % Return the first posterior/prior and initial values
			...
			initial_values =  [a(1) a(2) a(3)*a(4) a(3)*a(5)*a(7) a(5)*a(8) p.alpha(1) p.beta(1) p.t0 eps 0 0.25 0 0.25 0 0.25];
