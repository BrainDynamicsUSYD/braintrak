classdef gab_mass < bt.model.template
	...
	methods
		function self = gab_mass() % Constructor
			self.name = 'model_gab_mass';
			self.n_params = 9;
			...
			self.initial_step_size = 2*[0.4  0.4  10 10  0.2  5  40  0.005  0.05];
		...
		function [chisq,P] = objective(self,pars) % Calculate the objective
		    ...
		    d=(1.*(1-stab_L.*pars(2))-stab_L.*pars(1)).*(1-stab_L.*stab_L.*pars(5))-stab_L.*stab_L.*self.stab_Mtot.*(pars(3) + stab_L.*pars(4));      
		    ...
		    q2re2 = (1 - 1./(Jei_oneminus).*(spec_L.*pars(1) + ((spec_L.*spec_L.*pars(3) + spec_L.*spec_L.*spec_L.*pars(4)).*self.spec_Mtot)./(Jsrs_oneminus)));
		    ...
		    P = abs(T_prefactor./(q2re2)).^2; % For use with the efficient way
		...
		function [initial_values,prior_pp] = initialize_fit(self,target_f,target_P) % Return the first posterior/prior and initial values
			...
			initial_values =  [a(1) a(2) a(3)*a(4) a(3)*a(5)*a(7) a(5)*a(8) p.alpha(1) p.beta(1) p.t0 eps];
